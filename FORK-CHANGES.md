...
本文档列出相对上游 `main` 分支的全部改动，便于同步时核对和冲突处理。

对比基准：`upstream/main`（clash-verge-rev/clash-verge-rev:main）
...
- **同步方式**：`git rebase upstream/main`，不产生 merge commit；
...
## Commit 列表（相对 upstream/main，主题）

```
chore(fork): consolidate fork build and sync automation
```

fork 只有以上 commit（rebase 会改写 SHA，因此按主题列出）：

- `chore(fork): consolidate fork build and sync automation`：macOS 构建工作流、
  CI Tauri 配置、上游同步工作流、DNS 追加脚本、本文档；构建结束后向
  release 正文写入本次构建信息（上游提交列表、时间、hash）

改动文件清单见 `git diff --name-status upstream/main main`。
